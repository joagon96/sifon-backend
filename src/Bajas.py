...
def bajaRepartidor():
    ...
    data = getQueryData(
        "SELECT nomapeRep FROM Repartidor WHERE idRepartidor = ?", (idRepartidor2,))
    hab = 0
    executeQuery(
        "UPDATE Repartidor SET habilitadoRep=? WHERE idRepartidor = ?", (hab, idRepartidor2,))
    executeQuery("DELETE FROM Usuario WHERE usuario=?",
                 (data[0]['nomapeRep'],))
...
def bajaZona():
    ...
    hab = 0
    executeQuery("UPDATE Zona SET habilitado=? WHERE idZona = ?",
                 (hab, idZona,))
